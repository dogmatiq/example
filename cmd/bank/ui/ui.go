...
// Run starts an interactive user interface.
func Run(db *sql.DB, x dogma.CommandExecutor) {
	as := appState{
		db:       db,
		executor: x,
...
func (as *appState) execute(m dogma.Message) {
	err := as.executor.ExecuteCommand(
		context.Background(),
		m,
	)
	if err != nil {
...
func (as *appState) time() time.Time {
	return time.Now()
}
